...
// the game while you're rifling. Two states suspend auto-hide:
//   - pinned:    the player is lining up a throw; never yank the card away
//   - mouseMode: the player is interacting with the overlay itself
// The manual toggle hotkey keeps working either way; this only reacts to
// context changes from GSI.

function visibilityAction(ctx, { visible, pinned, mouseMode, autoShow }) {
  if (!autoShow) return null;
  const equipped = !!(ctx && ctx.equippedGrenade);
  if (equipped && !visible) return 'show';
  if (!equipped && visible && !pinned && !mouseMode) return 'hide';
  return null;
}
